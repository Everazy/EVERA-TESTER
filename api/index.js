...
// ── Rate limiting ─────────────────────────────────────────────
const limiterUmum = rateLimit({ windowMs: 15 * 60 * 1000, max: 60, standardHeaders: true, legacyHeaders: false });
const limiterAdmin = rateLimit({ windowMs: 15 * 60 * 1000, max: 20 });
const limiterBayar = rateLimit({ windowMs: 60 * 1000, max: 10 });
app.use("/api/admin", limiterAdmin);
app.use("/api/payments/create", limiterBayar);
...
async function cekStatusSakurupiah(merchantRef) {
  ...
  const sig     = crypto.createHash("md5").update(`${API_ID}${merchantRef}${API_KEY}`).digest("hex");
...
function deliverAkun(merchantRef) {
  ...
  orders[merchantRef] = {
    ...order, delivered: true, akunDikirim,
    deliveredAt: new Date().toISOString(), status: "sukses",
  };
  tulisJSON(ORDER_FILE, orders);
...
app.post("/api/payments/create", async (req, res) => {
  try {
    if (!API_ID)  throw new Error("SAKURUPIAH_API_ID belum diisi di env");
    if (!API_KEY) throw new Error("SAKURUPIAH_API_KEY belum diisi di env");
    if (!CB_URL)  throw new Error("SAKURUPIAH_CALLBACK_URL belum diisi di env");
    if (!RET_URL) throw new Error("SAKURUPIAH_RETURN_URL belum diisi di env");
    ...
    form.set("return_url",   `${RET_URL}?ref=${merchantRef}&produk=${encodeURIComponent(namaProduk)}`);
    ...
    if (!response.ok) return res.status(response.status).json({ ok: false, message: data?.message || "Sakurupiah error", data });

    const checkoutUrl = extractCheckoutUrl(data);
    if (!checkoutUrl) return res.status(502).json({ ok: false, message: "checkout_url tidak ditemukan", data });
...
app.get("/api/payments/status/:ref", async (req, res) => {
  ...
  if (order.delivered)         return res.json({ ok: true, status: "sukses", akun: order.akunDikirim });
  if (order.status === "stok_habis") return res.json({ ok: false, status: "stok_habis", message: "Stok habis, hubungi admin" });
  if (order.status === "expired")    return res.json({ ok: false, status: "expired", message: "Pembayaran kadaluarsa" });

  let statusData;
  try { statusData = await cekStatusSakurupiah(ref); }
  catch { return res.status(500).json({ ok: false, message: "Gagal cek ke Sakurupiah" }); }

  const kode   = String(statusData?.status || statusData?.data?.status || "");
  const pesan  = String(statusData?.message || "").toLowerCase();
  const isPaid = kode === "00" || kode === "success" || kode === "paid"
    || pesan.includes("sukses") || pesan.includes("paid") || pesan.includes("berhasil");
  const isExp  = kode === "02" || pesan.includes("expired") || pesan.includes("kadaluarsa");

  if (isExp) {
    orders[ref].status = "expired"; tulisJSON(ORDER_FILE, orders);
    return res.json({ ok: false, status: "expired", message: "Pembayaran kadaluarsa" });
  }
  if (!isPaid) return res.json({ ok: false, status: "pending", message: "Menunggu pembayaran" });

  const result = deliverAkun(ref);
  if (result.ok) return res.json({ ok: true, status: "sukses", akun: result.akun });
  return res.json({ ok: false, status: "stok_habis", message: result.message });
});

// ── 3. Callback dari Sakurupiah ───────────────────────────────
app.get("/api/payments/callback",  (_req, res) => res.json({ ok: true }));
app.post("/api/payments/callback", (req, res) => {
  console.log("📩 CALLBACK:", req.body);
  const ref = req.body?.merchant_ref || req.body?.ref;
  if (ref) { const r = deliverAkun(ref); console.log("AUTO-DELIVER callback:", ref, r.ok); }
...
app.post("/api/admin/login", (req, res) => {
  ...
  if (!cfgEmail || (!process.env.ADMIN_PASSWORD && !process.env.ADMIN_PASSWORD_SHA256))
    return res.status(500).json({ ok: false, message: "Kredensial admin belum diatur di env" });
...
// ── 5. Stok ───────────────────────────────────────────────────
app.get("/api/stok", adminAuth, (_req, res) => res.json({ ok: true, data: bacaJSON(STOK_FILE, {}) }));

app.get("/api/stok/:kode", (req, res) => {
  const stok = bacaJSON(STOK_FILE, {});
  res.json({ ok: true, jumlah: stok[req.params.kode]?.akun?.length || 0 });
});
...
app.post("/api/orders/:ref/deliver", adminAuth, (req, res) => {
  const { akun } = req.body || {};
  const orders   = bacaJSON(ORDER_FILE, {});
  if (!orders[req.params.ref]) return res.status(404).json({ ok: false, message: "Order tidak ditemukan" });
  orders[req.params.ref] = {
    ...orders[req.params.ref], delivered: true, akunDikirim: akun,
